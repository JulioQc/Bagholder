...
```
{
  ...
  "expect":   {"kpi": ..., "trades": [...], "positions": [...], "cashflowHoldings": [...]}
}
```

`expect` holds only the fields listed in `make_fixtures.py` (`TRADE_KEYS`, `KPI_KEYS`, `POSITION_KEYS`, `HOLDING_KEYS`), floats rounded to six places, lists sorted as the generator sorts them. The meaning of every field is in `SPEC.md`.
