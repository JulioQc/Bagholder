...
**Source health.** Every request to a source records its outcome, and the menu lists each source touched since start with its last outcome: `answered 2 min ago`, or the failure in words with its time (`could not be reached`, `refused the request (too many)`, `answered with an error (503)`). A symbol a source does not carry is not a failure of the source.
...
A version is a GitHub release tagged `vMAJOR.MINOR.PATCH`; commits alone are not versions. `APP_VERSION` in `bagholder.py` is bumped in the commit a release is cut from, and the running version is shown in the header as small muted text to the right of the Bagholder wordmark; nothing about versions appears in the menu. Once a day, and at start, the app asks GitHub for the latest release; when its tag is newer than the running version the header shows, beside the version at the top left, an "Update to vX.Y.Z" button when the app can install it itself, else an "Update available" link to that release.
