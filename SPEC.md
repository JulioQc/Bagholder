...
**Cancel.** Asks once (`Cancel order`, naming the order; `Keep order` returns). The cancel goes to Wealthsimple by the external id; on acceptance the row reads `Cancelling` until the next read says `Cancelled`, and the header reads `Cancel sent · Buy 1 QNC at 1.76 limit` for ten seconds; a refusal keeps the row as it was and puts Wealthsimple's reason in the header in red. Under the development setting `BAGHOLDER_DRY_ORDERS=1` the app never writes to Wealthsimple, and the button answers `Orders are off`.
