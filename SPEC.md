...
- **Open**: orders still resting at Wealthsimple, and filled entries whose bracket is still live, newest first, under `Open`.
...
**Bracket rows.** Under an entry with a bracket, one row per leg: a dot (accent while the leg is live), `Stop loss` with `Market at 157.13` or `Trailing 5% · 498.10 now`, and `Take profit` with `Limit at 181.94`; at the right the leg's state and, while the bracket is live, `Adjust`. States: `Waiting for fill`; once armed `Active` (a stop resting at Wealthsimple), `Watching` (a stop watched here), `Working` (the target watched), `Arming`, `Cancelling`; `Placed` when the target's limit sell is resting; at the end `Filled` on the leg that exited and `Cancelled` on the other; `Off` or `Failed` when the bracket ended without exiting. Adjust opens the level in place (Stop price, Trail or Trail %, Limit price) with Save, Remove and Cancel: Save moves the leg (a resting stop is cancelled and placed again at the new level by the engine's next check; a placed target order is cancelled and the target watched again), Remove takes the leg off (its resting order cancelled), and a bracket with both legs removed ends. The header reads `Stop loss moved · NVDA`, `Take profit removed · NVDA`, for ten seconds.
