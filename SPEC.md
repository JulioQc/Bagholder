...
- **Open**: the orders resting at Wealthsimple and, as a card of its own, each armed bracket, newest first together. A bracket still waiting for its entry to fill shows as leg rows on that pending order's card; once armed it is its own card, titled by the position it protects (`5 QNC`) with a `Bracket` pill, `Bought 1.75 · Trading (Margin)` and the position's value, then its two leg rows, and in the foot when it armed and `Cancel`, which asks once (`Cancel bracket`, naming the position and both legs; `Keep bracket` returns), cancels the bracket's resting orders at Wealthsimple and stops watching; the header reads `Bracket cancelled · 5 QNC` for ten seconds. The bracket's own resting stop is not a second card.
...
**Bracket rows.** On a bracket's card, one row per leg: a dot (accent while the leg is live), `Stop loss` with `Market at 157.13` or `Trail 5% · 498.10` (the current level), and `Take profit` with `Limit at 181.94`; at the right the leg's state and, while the bracket is live, `Adjust`. States say where the leg lives: `Waiting for fill` before the entry fills; `At Wealthsimple` when the leg is a resting order there (the stop once placed; the target's limit sell once the bid has reached it); `Watching` when Bagholder itself watches the quote for it (the target until the bid reaches it; a stop for a security Wealthsimple takes no stop order on); `Placing` and `Cancelling` while a request is out, `Retrying · <reason>` after a refused one; at the end `Filled` on the leg that exited and `Cancelled` on the other; `Off` when the bracket ended without exiting. Adjust opens the level in place (Stop price, Trail or Trail %, Limit price) with Save, Remove and Cancel: Save moves the leg (a resting stop is cancelled and placed again at the new level by the engine's next check; a placed target order is cancelled and the target watched again), Remove takes the leg off (its resting order cancelled), and a bracket with both legs removed ends. The header reads `Stop loss moved · NVDA`, `Take profit removed · NVDA`, for ten seconds.
