...
**Brackets after the fill.** A submitted ticket with a stop loss or a take profit becomes a bracket that waits for the entry to fill. Every five seconds while the app runs connected, each live bracket is checked, in this order: what Wealthsimple and the book say (an exit of the bracket's own filled ends it; a stop cancelled at Wealthsimple by hand leaves only the target watched; a stop that expires, a Day stop at the close or a good-till-cancelled one after Wealthsimple's ninety days, is placed again at the same level, so the ticket's time in force never leaves the position unprotected; a target order that expires or is cancelled puts the stop back; a position sold elsewhere is never inferred from Wealthsimple's balances, which have omitted a held position on one read and listed it on the next: a bracket is ended only by its own exits, by the person, or by Wealthsimple refusing an exit for shares that are not there, which the leg then shows), then the arming, then the watching.
...
- *The target.* When the bid reaches the target the resting stop is cancelled first; the stop is read back on every check, and the moment Wealthsimple confirms the cancel the limit sell at the target is placed. The stop stays in force until the cancel is confirmed. Nothing in the engine waits longer than it must: there is no pacing between its requests beyond the five-second check.
...
A side panel, not a tab: the header carries an Orders button (a receipt icon) between the sync status and the filter button, with a badge counting the open orders when there are any; it opens the panel over the page in the ticket's frame (400 px, the right edge, a scrim), as does ⌘O (Ctrl+O elsewhere), which pressed again closes it. Esc, the scrim and the × close it; Esc closes an open editor first. The panel lists every order the ticket submitted and every order Wealthsimple reports as pending from anywhere, its own app included, as cards under a segmented control:

- **Open**: the orders resting at Wealthsimple and, as a card of its own, each armed bracket, newest first together. A bracket still waiting for its entry to fill shows as leg rows on that pending order's card; once armed it is its own card, `Bracket 5 QNC` with its state as the pill, `Bought 1.75 · Trading (Margin)` and the position's value, then its two leg rows, and in the foot when it armed, `Edit`, and `Cancel`, which asks once (`Cancel bracket`, naming the position and both legs; `Keep bracket` returns), cancels the bracket's resting orders at Wealthsimple and stops watching; the header reads `Bracket cancelled · 5 QNC` for ten seconds. The bracket's own resting stop is not a second card.
- **Filled**: filled orders under `Today` and `Earlier`, the order and its fill only.
- **Cancelled**: cancelled, expired, rejected, failed and not-sent orders under `Cancelled and rejected`.

**A card.** First line: the side word (`Buy` green, `Sell` red), the quantity and the symbol, with a status pill at the right: `Pending` or `Partially filled` (accent), `Cancelling`, `Filled` (green), `Cancelled`, `Expired`, `Not sent` (grey), `Rejected`, `Failed` (red). Second line, muted: `Limit 165.40 · Day · Trading (Margin)` (`Market · TFSA` for a market order; `Filled 512.60 · Trading (Margin)` once filled), with the order's value at the right in the instrument's currency: quantity × the limit (the stop for a stop order; `≈` and the fill for a market order), contracts × 100 for an option, and what it filled for once filled. A third line when there is something to say: `40 of 100 filled · avg 64.50` on a partial fill, `250 filled · avg 36.85` on a fill, Wealthsimple's reason on a rejection. The card's foot carries when it was placed at the left, `Today 9:42 AM` or `Sep 8, 3:12 PM`, and, while the order is open, `Edit` and `Cancel` at the right. A bracket's own exit orders are never cards: the leg rows on the bracket's card are their presentation.

**One grammar for every card.** Row one: what it is, then a status pill at the right, always a state and never a kind (`Buy 5 QNC · Pending`, `Bracket 5 QNC · Armed`, the first word coloured: green Buy, red Sell, accent Bracket). Row two: how it was placed at the left, its value at the right. Then the detail rows. Then the foot: when, at the left; at the right, on every live card, `Edit` and `Cancel` in that order, Edit dimmed rather than absent where Wealthsimple cannot take a change (a stop order); a finished card has no actions. Nothing acts from inside a row.

**Bracket rows.** On a bracket's card, one row per leg: a dot (accent while the leg is live), `Stop loss` with `Market at 157.13` or `Trail 5% · 498.10` (the current level), and `Take profit` with `Limit at 181.94`; at the right the leg's state. The bracket's pill is `Waiting for fill`, `Armed`, `Firing` or `Target placed`. States say where the leg lives: `Waiting for fill` before the entry fills; `At Wealthsimple` when the leg is a resting order there (the stop once placed; the target's limit sell once the bid has reached it); `Watching` when Bagholder itself watches the quote for it (the target until the bid reaches it; a stop for a security Wealthsimple takes no stop order on); `Placing` and `Cancelling` while a request is out, `Retrying · <reason>` after a refused one; at the end `Filled` on the leg that exited and `Cancelled` on the other; `Off` when the bracket ended without exiting. Edit on a bracket opens one editor in place with both legs' levels (Stop price, or Trail / Trail %; Limit price), `Remove stop loss` and `Remove take profit` beneath them, Save and Cancel. Save moves whichever leg changed (a resting stop is cancelled and placed again at the new level by the engine's next check; a placed target order is cancelled and the target watched again); Remove takes the leg off, its resting order cancelled, and a bracket with both legs removed ends. The header reads `Bracket changed · 5 QNC`, `Stop loss removed · 5 QNC`, for ten seconds.

**Edit and Cancel.** Edit opens Shares and, on a limit or stop-limit order, Limit price in place; Save sends Wealthsimple's own modify (`SoOrdersOrderModify` with the external id, `newLimitPrice`, `newQuantity`), and the header reads `Order changed · Buy 30 NVDA at 164.00 limit`. A stop order has no Edit, since Wealthsimple's modify carries no stop price. Cancel asks once (`Cancel order`, naming the order; `Keep order` returns), goes to Wealthsimple by the external id, and the card reads `Cancelling` until the next read says `Cancelled`; the header reads `Cancel sent · Buy 1 QNC at 1.76 limit` for ten seconds, and a refusal keeps the card and puts Wealthsimple's reason in the header in red.
