...
| Coinbase Exchange | Daily and hourly candles of each traded or held crypto asset: the asset's market in the position's currency when one trades there, else its USD market converted at the Bank of Canada rate of the bar's day; a bar whose day has no published rate is dropped, never guessed | On demand for the chart; the archive keeps them |
...
| Crypto | Coinbase Exchange candles, in the position's currency (see the sources table) | 1H, 4H, 1D, 1W, 1M for every trade | Full history |
