...
**Cashflow.** Tiles; Cashflow (the monthly bars in the accent colour with the month's margin interest drawn over them from the same baseline at the same width in the negative colour, the legend at the card's top right; while a bar is pressed, the month's Distributions, Margin interest and Net in a small card of its own, outlined and shadowed, floating over the bars at the top left under the title, so nothing else moves); Allocation (the donut by projected monthly income, no switch); Positions, each row the symbol with `shares · avg · dist × freq` under it on the left, the month's payout over the yield on cost on the right, then Ex-Div and Pay Day; History, each row the symbol, `date · qty × per`, the amount and its currency, without the payment kind.
