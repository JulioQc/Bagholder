...
All pages share the header (brand and version, sync status, filter, menu), the tab row and the current filter set. Every page lays its tiles and card rows on one six-column grid with a 14 px gap: a tile is one column, a card a whole number of columns (Dashboard rows 4 + 2, Portfolio 2 + 4, Cashflow 3 + 3), so card edges are tile edges. With nothing open and nothing being typed, the Left and Right arrow keys move between the tabs in their order, Dashboard to Orders, stopping at the ends. Until the page has heard from the server (a page loaded while Bagholder is still starting), the sync status is a spinner alone: `Not connected` and `Synced` are only ever the server's answer. A short-lived notice from an action (session refreshed, trade added, folder scanned, or an error) takes the sync status's place for four seconds; it is red only when it reports an error. A sync error appears once: on the "No activity yet" page it is shown in full beneath the page's button and the header keeps its plain status; everywhere else it takes the header's sync status.
...
**Order.** Buy or Sell, full width. Then a two-column grid: Account (the open self-directed accounts that trade securities; crypto, predictions and managed accounts are not offered; a Sell opens on the account holding the position, a Buy on the margin account when there is one) · Shares; Order type (only the types Wealthsimple allows for that security, of Market, Limit, Stop, Stop limit) · Limit price (Limit and Stop limit); Stop price (Stop and Stop limit) takes the next free cell; Time in force (Day · Good till cancelled; hidden for a Market order unless a bracket is on, since the brackets use it too) · Amount. Shares and Amount are one quantity seen twice: typing an amount sets the whole shares it buys at the working price, typing shares sets the amount; the working price is the ask on a Market buy, the bid on a Market sell, the stop price on a Stop order, else the limit price. A field holds what was typed while it has focus and is formatted when it loses it (Enter commits it). Defaults: one share (a Sell, the position's quantity), the limit at the last price, an entry stop 2% past the last price.

**Stop loss and Take profit.** Only on a Buy: a Sell leaves nothing to protect, so neither section is shown. Each has Add or Remove at its right and is open by default, and the one Time in force covers the order and both. Stop loss: Type `Stop` (a Stop price, 5% below the working price by default) or `Trailing stop` (`Trail` and `Trail by`: `Percent of price` or `Amount per share`; 5% by default); the line beneath reads `Sells at 157.13` or `Starts at 157.13` and, in red, the loss if it fires with its percentage of the entry, `−$206.75 (−5.0%)`. Take profit: a Limit price (10% above by default); `Sells at 181.94` and, in green, the gain, `+$413.50 (+10.0%)`. Switching Buy and Sell recomputes the defaults for the new direction.

**Brackets after the fill.** A submitted ticket with a stop loss or a take profit becomes a bracket that waits for the entry to fill. Every five seconds while the app runs connected, each live bracket is checked, in this order: what Wealthsimple and the book say (an exit of the bracket's own filled ends it; a stop cancelled at Wealthsimple by hand leaves only the target watched; a target order that expires or is cancelled puts the stop back; a position no longer in the account, seen on a balances read newer than the arming, ends it and cancels its resting orders), then the arming, then the watching.

- *Arming.* When the entry reads filled, or ended with a partial fill, the bracket arms for the filled quantity, so the stop is placed on the read after the fill, up to 30 seconds later, never in the same breath as the entry. A stop loss goes to Wealthsimple as its own stop order for that quantity, with the ticket's time in force, when the security's allowed order types include a stop; when they do not, the stop is watched here (the row says so) and fires as a market sell when the bid reaches it. A take profit is always watched here.
- *Trailing.* The level follows the high of Wealthsimple's quote: high minus the trail (a percent of the high, or the amount). The resting stop is moved only when the new level is at least half a percent above the current one and at most once a minute, since a move is a cancel and a new stop order at Wealthsimple; the level in the table moves with it.
- *The target.* When the bid reaches the target the resting stop is cancelled first; once Wealthsimple confirms the cancel the limit sell at the target is placed. The stop stays in force until the cancel is confirmed.
- *Spacing.* The engine never sends two order writes, a new order or a cancel, within ten seconds of each other, counting from anything the app sent, the ticket included: an action that comes due sooner waits for a later check. Nothing the engine does is therefore ever placed within seconds of the entry, or of another exit. A person's own ticket and Cancel are not held.
- *Failures.* An exit Wealthsimple rejects is tried again on the next check, three times in all, then the bracket reads `failed` with the reason and is left alone; every step is a line on the terminal (`bagholder bracket: …`). With `BAGHOLDER_LIVE_ORDERS` off nothing is placed: the request each step would send is printed once, and the bracket keeps waiting. A bracket only fires while the app is running and connected; with the app closed the resting Wealthsimple stop is the only protection.

**Review.** `Buy 25 NVDA` with the side coloured, then `Limit 165.40 · Day · Trading` (a Market order: `Market · Trading`). Rows: Stop loss `Market at 157.13`, `Trails 5% under the high · starts at 157.13`, or `None`; Take profit `Limit at 181.94` or `None`; then At risk (the stop loss line's loss, red), Target (the take profit's gain, green), Risk / reward `1:2` (one decimal only when it is not zero; `—` without both brackets), Position size `5.6% of net asset value` (the order's cost in CAD at today's rate over the net liquidation value of every open account), and either Available margin after (a margin account: its available margin minus the cost in CAD times the security's client margin rate from Wealthsimple, the full cost when the rate is unknown; red when negative) or Cash after (any other account: the account's cash in the order's currency, minus the cost); a Sell adds instead of subtracting. Last, Estimated cost (Estimated proceeds on a Sell): the quantity times the working price, in the security's currency, `≈` before it for a Market order. Back returns to the form as it was.

**Submit.** The request is the one Wealthsimple's web app sends (`SoOrdersOrderCreate`: account, security, execution type, side and quantity, prices, time in force, and an external id the app makes up and keeps). The ticket is written to the store before anything is sent, then updated with Wealthsimple's order id, or its rejection, or the failure; every submit is a row, and the rows are the user's own record: `Clear data` does not touch them. Orders reach Wealthsimple only when the app runs with `BAGHOLDER_LIVE_ORDERS=1`; otherwise a submit records the ticket, prints the request it would have sent on the terminal, and the header says so. After a submit the panel closes and the header reads, for ten seconds (it names the whole order, so longer than the page's other notices), `Order placed · Buy 25 NVDA at 165.40 limit, stop 157.13, target 181.94` (`at market` for a Market order; the stop and target clauses only when set), or `Not sent (orders are off) · …` when the switch is off. A rejection or failure stays in the review step with its reason. Nothing else on the page changes.
...
One card, the Orders table: every order the ticket submitted, and every order Wealthsimple reports as pending from anywhere (its own app included), newest first with the open ones on top. Columns: Placed (local date and time) · Symbol · Side (`Buy` green, `Sell` red) · Type (Market, Limit, Stop, Stop limit) · Qty · Price (the limit; the stop for a Stop order; `Stop 1.60 · 1.55` for a Stop limit; `—` for Market) · Time in force (`Day`, `GTC`, `—` when Wealthsimple has not said) · Account · Brackets · Status · a `Cancel` button on an open order, or `Cancel bracket` on a filled entry whose bracket is live. Sortable by any column; open orders stay above the rest whatever the sort.

The Brackets cell reads the bracket's current levels and state: `Stop 1.67 · Target 1.94 · waiting for the fill`, then `armed` (`armed · stop watched here` when Wealthsimple takes no stop order for the security), `firing`, `target order placed`, and at the end `stopped`, `target hit`, `off · <why>` or `failed · <reason>`; a trailing stop shows its current level, `Trail 5% at 1.62`. The bracket's own exit orders are rows too, `Stop loss for QNC` and `Take profit for QNC` in that cell, with their own status and Cancel. `Cancel bracket` asks once, cancels the bracket's resting orders at Wealthsimple and stops watching; the entry is not touched.

**Status** is Wealthsimple's, read back through the order's external id on the `TR` branch: `Pending` (their NEW, PENDING_SUBMISSION, PENDING_REVIEW, PENDING_FUND_TRANSFER, SUBMITTED, PLACED, CONTINGENT; `Pending · filled 10 @ 1.76` when partly filled), `Cancelling` (CANCEL_PENDING), `Filled 25 @ 165.38`, `Cancelled`, `Expired`, `Rejected · <their reason>`; and the app's own states before Wealthsimple has answered: `Sent`, `Not sent` (a dry submit), `Failed · <reason>`. Open orders are shown in the accent colour, rejections and failures in red.

**Refresh.** Every open order is read back at once after it is sent and every 30 seconds while the app runs connected, and the pending-order feed is read with it, so an order placed in Wealthsimple's app appears as a row (its time in force learned on the next read); with nothing open the feed alone is read every five minutes. The tab asks the server for the list every ten seconds while it is up, and its title row says when Wealthsimple was last asked (`checked 2 min ago`) and `orders are off` when the switch is. A read that fails is counted on the terminal (`bagholder orders: …`) and changes nothing.

**Cancel.** Asks once (`Cancel order`, naming the order; `Keep order` returns). The cancel goes to Wealthsimple by the external id; on acceptance the row reads `Cancelling` until the next read says `Cancelled`, and the header reads `Cancel sent · Buy 1 QNC at 1.76 limit` for ten seconds; a refusal keeps the row as it was and puts Wealthsimple's reason in the header in red. Only with `BAGHOLDER_LIVE_ORDERS=1`: without the switch the app never writes to Wealthsimple, and the button answers `Orders are off`.
