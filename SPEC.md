...
- **Open**: orders still resting at Wealthsimple under `Open`, then filled entries whose bracket is still live under `Brackets working`, newest first within each.
- **Filled**: filled orders under `Today` and `Earlier`.
- **Cancelled**: cancelled, expired, rejected, failed and not-sent orders under `Cancelled and rejected`.

**A card.** First line: the side word (`Buy` green, `Sell` red), the quantity and the symbol, with a status pill at the right: `Pending` or `Partially filled` (accent), `Cancelling`, `Filled` (green), `Cancelled`, `Expired`, `Not sent` (grey), `Rejected`, `Failed` (red). Second line, muted: `Limit 165.40 · Day · Trading (Margin)` (`Market · TFSA` for a market order; `Filled 512.60 · Trading (Margin)` once filled), with the order's value at the right in the instrument's currency: quantity × the limit (the stop for a stop order; `≈` and the fill for a market order), contracts × 100 for an option, and what it filled for once filled. A third line when there is something to say: `40 of 100 filled · avg 64.50` on a partial fill, `250 filled · avg 36.85` on a fill, Wealthsimple's reason on a rejection. The card's foot carries when it was placed at the left, `Today 9:42 AM` or `Sep 8, 3:12 PM`, and, while the order is open, `Edit` and `Cancel` at the right. The bracket's own exit orders are not cards; they show as the entry's bracket rows.

**Bracket rows.** Under an entry with a bracket, one row per leg: a dot (accent while the leg is live), `Stop loss` with `Market at 157.13` or `Trailing 5% · 498.10 now`, and `Take profit` with `Limit at 181.94`; at the right the leg's state and, while the bracket is live, `Adjust`. States say where the leg lives: `Waiting for fill` before the entry fills; `At Wealthsimple` when the leg is a resting order there (the stop once placed; the target's limit sell once the bid has reached it); `Watching` when Bagholder itself watches the quote for it (the target until the bid reaches it; a stop for a security Wealthsimple takes no stop order on); `Placing` and `Cancelling` while a request is out; at the end `Filled` on the leg that exited and `Cancelled` on the other; `Off` or `Failed` when the bracket ended without exiting. Adjust opens the level in place (Stop price, Trail or Trail %, Limit price) with Save, Remove and Cancel: Save moves the leg (a resting stop is cancelled and placed again at the new level by the engine's next check; a placed target order is cancelled and the target watched again), Remove takes the leg off (its resting order cancelled), and a bracket with both legs removed ends. The header reads `Stop loss moved · NVDA`, `Take profit removed · NVDA`, for ten seconds.
