...
**One grammar for every card.** Row one: what it is, with the first word coloured (green `Buy`, red `Sell`, accent `Bracket`), and the order's value at the right in the instrument's currency (quantity × the limit, the stop for a stop order, `≈` and the fill for a market order, contracts × 100 for an option, what it filled for once filled). A bracket is titled by its exchange and symbol, `Bracket NYSE: QNC`, without the quantity. Row two: how it was placed, `Limit 165.40 · Day`, `Market`, `Filled at 1.75`, or for a bracket `Bought 5 at 1.75`; the account is never on the card, it is the panel's heading. No status pills: the tab already says pending or filled, and the ended tab names the state on the second line. Then the detail rows: `40 of 100 filled · avg 64.50` on a partial fill, `250 filled · avg 36.85` on a fill, Wealthsimple's reason on a rejection; on a bracket, one row per leg. Then the foot: when, at the left (`Today 9:42 AM`, `Sep 8, 3:12 PM`); at the right, on every live card, `Edit` and `Cancel` in that order, Edit dimmed rather than absent where Wealthsimple cannot take a change (a stop order); a finished card has no actions. Nothing acts from inside a row. A bracket's own exit orders are never cards: the leg rows are their presentation.
