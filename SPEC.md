...
**Brackets after the fill.** A submitted ticket with a stop loss or a take profit becomes a bracket that waits for the entry to fill. Every five seconds while the app runs connected, each live bracket is checked, in this order: what Wealthsimple and the book say (an exit of the bracket's own filled ends it; a stop cancelled at Wealthsimple by hand leaves only the target watched; a stop that expires, after Wealthsimple's ninety days or a Day stop from before every exit went good till cancelled, is placed again at the same level, good till cancelled; a target order that expires or is cancelled puts the stop back; a position sold elsewhere is never inferred from Wealthsimple's balances, which have omitted a held position on one read and listed it on the next: a bracket is ended only by its own exits, by the person, or by Wealthsimple refusing an exit for shares that are not there, which the leg then shows), then the arming, then the watching.
