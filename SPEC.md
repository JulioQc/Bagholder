...
A side panel, not a tab: the header carries an Orders button (a receipt icon) between the sync status and the filter button, with a badge counting the open orders when there are any; it opens the panel over the page in the ticket's frame (400 px, the right edge, a scrim). Esc, the scrim and the × close it; Esc closes an open editor first. The panel lists every order the ticket submitted and every order Wealthsimple reports as pending from anywhere, its own app included, as cards under a segmented control:
