...
A side panel, not a tab: the header carries an Orders button (a receipt icon) between the sync status and the filter button, with a badge counting the pending orders when there are any; it opens the panel over the page in the ticket's frame (400 px, the right edge, a scrim), as does ⌘O (Ctrl+O elsewhere), which pressed again closes it. Esc, the scrim and the × close it; Esc closes an open editor first. Left and Right move between its tabs. ⌘K still opens the filter popover over the panel, so the accounts in scope can be changed without leaving it, and the panel follows the page's account filter: its heading row names the accounts in scope at the right, `All Accounts` when the filter names none, and every card and bracket outside them is left out. The panel lists every order the ticket submitted and every order Wealthsimple reports as pending from anywhere, its own app included, as cards under a segmented control:

- **Pending**: everything not yet filled, cancelled or rejected, under the heading `Pending orders`: the orders resting at Wealthsimple and, as a card of its own, each armed bracket, newest first together. A bracket still waiting for its entry to fill shows as leg rows on that pending order's card; once armed it is its own card.
- **Filled**: filled orders under `Filled orders`, grouped `Today` and `Earlier`, the order and its fill only.
- **Cancelled**: cancelled, expired, rejected, failed and not-sent orders under `Cancelled and rejected`, each with its state as a word at the right of its second line (`Rejected` in red).

**One grammar for every card.** Row one: what it is, with the first word coloured (green `Buy`, red `Sell`, accent `Bracket`), and the order's value at the right in the instrument's currency (quantity × the limit, the stop for a stop order, `≈` and the fill for a market order, contracts × 100 for an option, what it filled for once filled). A bracket is titled by its exchange and symbol, `Bracket NYSE: QNC`, without the quantity. Row two: how it was placed, `Limit 165.40 · Day`, `Market`, or how it went, `Filled 5 at 1.75`, and for a bracket `Bought 5 at 1.75`; the account is never on the card, it is the panel's heading. No status pills: the tab already says pending or filled, and the ended tab names the state on the second line. A third line only for what the second cannot say: `40 of 100 filled at 64.50` on a pending order partly filled, Wealthsimple's reason on a rejection; on a bracket, one row per leg. Then the foot: when, at the left (`Today 9:42 AM`, `Sep 8, 3:12 PM`); at the right, on every live card, `Edit` and `Cancel` in that order, Edit dimmed rather than absent where Wealthsimple cannot take a change (a stop order); a finished card has no actions. Nothing acts from inside a row. A bracket's own exit orders are never cards: the leg rows are their presentation.

**Bracket rows.** One row per leg, a dot (accent while the leg is live) and one sentence: `Stop loss at 157.13`, or `Stop loss at 162.40 · trailing 5%` (the current level), and `Take profit at 181.94`; at the right the leg's state. States say where the leg lives: `Waiting for fill` before the entry fills; `At Wealthsimple` when the leg is a resting order there (the stop once placed; the target's limit sell once the bid has reached it); `Watching` when Bagholder itself watches the quote for it (the target until the bid reaches it; a stop for a security Wealthsimple takes no stop order on); `Placing` and `Cancelling` while a request is out, `Retrying · <reason>` after a refused one; at the end `Filled` on the leg that exited and `Cancelled` on the other; `Off` when the bracket ended without exiting. Edit on a bracket opens one editor in place with both legs' levels (Stop price, or Trail / Trail %; Limit price), `Remove stop loss` and `Remove take profit` beneath them, Save and Cancel. Save moves whichever leg changed (a resting stop is cancelled and placed again at the new level by the engine's next check; a placed target order is cancelled and the target watched again); Remove takes the leg off, its resting order cancelled, and a bracket with both legs removed ends. The header reads `Bracket changed · QNC`, `Stop loss removed · QNC`, for ten seconds. Cancel on a bracket asks once (`Cancel bracket`, naming the symbol and both legs; `Keep bracket` returns), cancels the bracket's resting orders at Wealthsimple and stops watching; the header reads `Bracket cancelled · QNC`.

**Edit and Cancel on an order.** Edit opens Shares and, on a limit or stop-limit order, Limit price in place; Save sends Wealthsimple's own modify (`SoOrdersOrderModify` with the external id, `newLimitPrice`, `newQuantity`), and the header reads `Order changed · Buy 30 NVDA at 164.00 limit`. A stop order has no Edit, since Wealthsimple's modify carries no stop price. Cancel asks once (`Cancel order`, naming the order; `Keep order` returns), goes to Wealthsimple by the external id, and the card reads `Cancelling` on its second line until the next read says it is cancelled; the header reads `Cancel sent · Buy 1 QNC at 1.76 limit` for ten seconds, and a refusal keeps the card and puts Wealthsimple's reason in the header in red.
