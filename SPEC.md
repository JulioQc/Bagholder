...
**Allocation.** A donut of the open positions in scope by market value in CAD, the seven largest and `Other (N)`, coloured from the eight-colour palette; the centre reads `Market value` and the total, or the hovered slice's value and share; the legend is the symbols. No toggle.

**Holdings.** One row per open position: Symbol (with its account in muted text, since one symbol can be held in several accounts; `SHORT` on a short) · Book · Market · Unrealized (`+$20.05 (+3.4%)`) · Change ($) · Change (%), sorted by Unrealized; the two Change columns are the day's move on the position from its quote's change and percent change, `—` without a quote. Book, Market and the changes are in the position's currency.
