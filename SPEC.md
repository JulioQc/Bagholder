...
**Holdings.** One row per open position: Symbol (`SHORT` on a short; the account is the row's title text, not shown) · Avg (average cost per unit) · Last (the current price) · Book · Market · Unrealized (`+$20.05 (+3.4%)`) · Change ($) · Change (%), sorted by Unrealized; the two Change columns are the day's move on the position from its quote's change and percent change, `—` without a quote. Avg, Last, Book, Market and the changes are in the position's currency.
