...
- *The target.* When the bid reaches the target the resting stop is cancelled first; once Wealthsimple confirms the cancel the limit sell at the target is placed. That cancel and that limit sell are the one place two requests follow each other; the stop stays in force until the cancel is confirmed.
